...
void eval_micro_sequencer() {
    ...
    else if (GetIRD(CONTROL_STORE[currState])) NEXT_LATCHES.STATE_NUMBER= ((uint8_t) (CURRENT_LATCHES.IR >> 12))&0xF;
    else if (GetJGate(CONTROL_STORE[currState])) NEXT_LATCHES.STATE_NUMBER=CURRENT_LATCHES.JSave;
}

int16_t checkException(int* cStore){
    if((((CURRENT_LATCHES.IR&0xF000)>>13)==5)){
        NEXT_LATCHES.EXCV=5;return 0;}//unknown opcode
    else if(((NEXT_LATCHES.STATE_NUMBER==45) && (CURRENT_LATCHES.MAR%2)  //unaligned access
             && ((CURRENT_LATCHES.JSave!=24) && (CURRENT_LATCHES.JSave!=29)))){
        NEXT_LATCHES.EXCV=3;return 1;}
    else if((NEXT_LATCHES.STATE_NUMBER==48)&&(~CURRENT_LATCHES.MDR&0x8)&&( //protection exception
    ((CURRENT_LATCHES.IR&0xF000)!=0xF000)&&(CURRENT_LATCHES.PSR&0x8000))){
        NEXT_LATCHES.EXCV=4;return 1;
    }
    else if((NEXT_LATCHES.STATE_NUMBER==48)&&(~CURRENT_LATCHES.MDR&0x4)){ //page fault
        NEXT_LATCHES.EXCV=2;return 1;
    }//page fault
    return 0;
}
...
void eval_bus_drivers() {
    ...
    switch(gates){
        ...
        case 0x200:  BUS=CURRENT_LATCHES.PC-2;break;
        case 0x400:  BUS=(CURRENT_LATCHES.PTBR&0xFF00)+((CURRENT_LATCHES.MAR&0xFE00)>>8);break; //todo double check shf
        case 0x800:  BUS=(CURRENT_LATCHES.MDR&0x3E00) + (CURRENT_LATCHES.VA&0x00FF);break; //todo load MAR at this state
        case 0x1000: BUS=CURRENT_LATCHES.TMPPSR; break;
    }
    BUS=BUS&0xFFFF;
}
...
int16_t latchVA(int* cStore){if(GetLD_VA(cStore)){NEXT_LATCHES.VA=BUS&0xFFFF;}}
...
int16_t latchMDR(int* cStore){
    ...
    if(GetLD_MDRR(cStore)) NEXT_LATCHES.MDR|=0x1;
    if(GetLD_MDRM(cStore)){
        NEXT_LATCHES.MDR&=~0x2;
        int tmp = (CURRENT_LATCHES.IR&0xF000)>>12;
        NEXT_LATCHES.MDR|=2*((CURRENT_LATCHES.JSave==61)||(CURRENT_LATCHES.JSave==56)
...
int16_t latchTMPPSR(int* cStore){
    if(GetLD_TMPPSR(cStore)) NEXT_LATCHES.TMPPSR=BUS;
}
