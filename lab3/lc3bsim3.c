...
void cycle() {
    ...
    CURRENT_LATCHES = NEXT_LATCHES;CYCLE_COUNT++;
}
...
void get_command(FILE * dumpsim_file) {
    ...
    switch(buffer[0]) {
        ...
        case 'M':
        case 'm':
            scanf("%i %i", &start, &stop); // NOLINT(cert-err34-c)
...
void eval_micro_sequencer() {
    ...
    if (GetIRD(CONTROL_STORE[currState]) == 0) {
        switch (conds) {
            case 0:
                break;
            case 1:
...
void eval_bus_drivers() {
    ...
    switch(gates){
        ...
        case 0:  BUS=0; break;
        case 1:  evalALU(cStore);break;
        case 2:  BUS=CURRENT_LATCHES.PC&0xFFFF;break;
        case 4:  evalMDR(cStore);break;
        case 8:  evalMARMUX(cStore); break;
        case 16: evalShift(cStore);break;
    }
}
...
int16_t latchCC(int* cStore){
    if(GetLD_CC(cStore)){
        if((BUS&(1<<15))){NEXT_LATCHES.N=1;NEXT_LATCHES.Z=0;NEXT_LATCHES.P=0;}
        else if(BUS==0){NEXT_LATCHES.N=0;NEXT_LATCHES.Z=1;NEXT_LATCHES.P=0;}
...
int16_t latchREG(int* cStore){
    if(GetLD_REG(cStore)){
        int DR;
        if(GetDRMUX(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER])==1) DR=7;
        else DR = getBits(CURRENT_LATCHES.IR,3,9,0);
        NEXT_LATCHES.REGS[DR]=BUS&0xFFFF;
    }
}
int16_t latchBEN(int* cStore){
...
int16_t latchMDR(int* cStore){
    int isWrite = GetR_W(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]);
...
int16_t getSR1(int* cStore){
    if(GetSR1MUX(cStore)==0) return (int16_t) CURRENT_LATCHES.REGS[getBits(CURRENT_LATCHES.IR, 3, 9,0)];
...
int16_t getBits(int reg, int numBits, int start, int sext){
    ...
    for(int i = numBits; i<16; i++) out |= ((out&(1<<(numBits-1))&&1)<<i);
